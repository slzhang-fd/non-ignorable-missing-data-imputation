...
@torch.jit.script
def sample_eta_mixed_nonig_jit(
    response_cont,
    response_bin,
    W_beta_cont,
    W_beta_bin,
    ...
    mu_temp = (
        W_beta_cont.T @ torch.diag(sigma_sq_inv) @ (response_cont - x_cov_alpha_c).T
        + W_beta_bin.T @ (response_bin - 0.5).T
        + W_kappa.T @ xi.T
    )
...
def sample_xi_combined_jit(
    ...
    for i in range(n_responses):
        Sigma_xi_pos = torch.inverse(
            W_zeta.T @ torch.diag(Omega[i, :]) @ W_zeta + torch.eye(n_xi)
        )
...
class NonIgnorableImputerInfer:
    ...
    def sample_eta(self):
        ...
        self.eta = sample_eta_mixed_nonig_jit(
    ...
    def calcu_loglik(self):
        ...
        loglik += (self.z_variables * temp_z - torch.log(1 + torch.exp(temp_z))).sum()
        # log p(xi | eta)
...
class NonIgnorableImputerInfer_stand_alone:
    def __init__(self, responses, n_eta, n_xi, x_covs, params_hat_all):
        ...
        self.w_kappa_mask = params_hat_all["w_kappa_mask"]
    ...
    def calcu_loglik(
        ...
        W_kappa = torch.zeros(self.n_xi, self.n_eta)
        W_kappa[self.w_kappa_mask] = W_kappa_reduced
        temp = self.x_covs @ W_alpha.T + self.eta @ W_beta.tril().T
        # log p(y_cont | eta)
        loglik = (
            MultivariateNormal(
                torch.zeros(self.n_cont),
    ...
    def calcu_jacobian(self):
        inputs = (
            ...
            self.W_kappa[self.w_kappa_mask],
        )
        with torch.no_grad():
    ...
    def calcu_loglik_vec(
        ...
        temp_z = self.x_covs @ W_gamma.T + self.xi @ W_zeta.tril().T
        loglik_vec += (
            self.z_variables * temp_z - torch.log(1 + torch.exp(temp_z))
        ).sum(1)
        # log p(xi | eta)
    ...
    def calcu_loglik_closure(self, inputs_vec):
        # log p(y | eta)
        log_sigma, W_alpha, W_beta_reduced, W_gamma, W_zeta_reduced, W_kappa_reduced = (
            inputs_vec.split(
                [
                    self.n_cont,
