## Simulation Study II-1
## True model is non-ignorable, estimated imputation model is ignorable.
...
from depend_funcs_ignorable import (  # noqa: E402
    IgnorableImputerInfer,
    IgnorableImputerInfer_stand_alone,
...
model = IgnorableImputerInfer(
    responses_masked.clone(), n_eta, x_covs.clone(), initial_values
)
...
model_infer = IgnorableImputerInfer_stand_alone(
    responses_masked.clone(), n_eta, x_covs.clone(), params_hat_all
)
start_time = time.time()
...
# %% save results
with open("../res_folder/simu2-1/res_" + str(reps) + ".pkl", "wb") as f:
    pickle.dump(
        [
            responses_all,
            S_ij_all1,
